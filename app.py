...
class GenConfig:
    ...
    severe_ae_rate: float = 0.20
    baseline_window_days: int = 60
    visit_jitter_days: int = 3
    ae_mean_per_subject: float = 0.6
...
with st.sidebar:
    ...
    n_sites = st.slider("Number of sites", min_value=1, max_value=12, step=1, key="n_sites")
...
###### with prompt (does not work quite well)
# import io
# import json
# import zipfile
# from dataclasses import dataclass
...
#     severe_rate = snap(clampf(float(cfg.get("severe_rate", 0.2)), 0, 0.8), 0.05)
#     ae_mean = snap(clampf(float(cfg.get("ae_mean", 0.6)), 0, 3), 0.1)
#     dropout_rate = snap(clampf(float(cfg.get("dropout_rate", 0.1)), 0, 0.6), 0.05)
#     missed_visit_rate = snap(clampf(float(cfg.get("missed_visit_rate", 0.05)), 0, 0.3), 0.05)
#     missing_field_rate = snap(clampf(float(cfg.get("missing_field_rate", 0.02)), 0, 0.2), 0.02)

#     output_mode = str(cfg.get("output_mode", "VALID")).upper()
#     if output_mode != "INVALID":
#         output_mode = "VALID"
